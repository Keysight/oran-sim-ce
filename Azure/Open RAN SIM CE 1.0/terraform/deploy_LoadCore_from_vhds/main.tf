...
resource "azurerm_image" "loadcore_mdw_image" {
  name                      = "LoadCore-MDW-image-from-vhd"
  location                  = azurerm_resource_group.rg.location
  resource_group_name       = azurerm_resource_group.rg.name

  os_disk {
    os_type = "Linux"
    blob_uri = "${var.loadcore_mdw_blob_uri}"
    os_state = "Generalized"

  }
}

resource "azurerm_image" "loadcore_agent_image" {
  name                      = "LoadCore-Agent-image-from-vhd"
  location                  = azurerm_resource_group.rg.location
  resource_group_name       = azurerm_resource_group.rg.name

  os_disk {
    os_type = "Linux"
    blob_uri = "${var.loadcore_agent_blob_uri}"
    os_state = "Generalized"
  }
}
...
resource "azurerm_linux_virtual_machine" "loadcore_mdw" {
  ...
  source_image_id = "${azurerm_image.loadcore_mdw_image.id}"
...
resource "azurerm_linux_virtual_machine" "loadcore_agent1" {
  ...
  source_image_id = "${azurerm_image.loadcore_agent_image.id}"
